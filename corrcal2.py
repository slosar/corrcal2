...
sparse_mat_times_vec_c.argtypes = [
    ctypes.c_void_p,
    ctypes.c_void_p,
    ctypes.c_void_p,
    ctypes.c_int,
...
class sparse_2level:
    ...
    def __mul__(self, vec):
        ...
        sparse_mat_times_vec_c(
            self.diag.ctypes.data,
            self.vecs.ctypes.data,
            self.src.ctypes.data,
            n,
            nvec,
            nsrc,
            self.nblock,
            self.lims.ctypes.data,
            self.isinv,
            vec.ctypes.data,
            ans.ctypes.data,
        )
...
def get_chisq_dense(g, data, noise, sig, ant1, ant2, scale_fac=1.0, normfac=1.0):
    ...
    apply_gains_to_mat_c(
        cov.ctypes.data, g.ctypes.data, ant1.ctypes.data, ant2.ctypes.data, n // 2, n
    )
    ...
    nn = g.size / 2
    chisq = chisq + normfac * (
        (numpy.sum(g[1::2])) ** 2 + (numpy.sum(g[0::2]) - nn) ** 2
    )
...
def get_gradient_dense(g, data, noise, sig, ant1, ant2, scale_fac=1.0, normfac=1.0):
    ...
    gsd = sd.copy()
    apply_gains_to_mat_c(
        gsd.ctypes.data, g.ctypes.data, ant2.ctypes.data, ant1.ctypes.data, n // 2, 1
    )
