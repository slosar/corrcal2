...
u_org = umat[isok]
v_org = vmat[isok]
vis_org = numpy.random.randn(ant1_org.size) + numpy.complex(0, 1) * numpy.random.randn(
    ant1_org.size
)

noise_org = numpy.ones(u_org.size)

vis, u, v, noise, ant1, ant2, edges = corrcal2.grid_data(
    vis_org, u_org, v_org, noise_org, ant1_org, ant2_org
)
for i in range(len(edges) - 1):
    mystd = numpy.std(u[edges[i] : edges[i + 1]]) + numpy.std(
        v[edges[i] : edges[i + 1]]
    )
